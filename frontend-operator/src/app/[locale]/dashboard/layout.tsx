...
const getNavigation = (t: any) => {
    const mainNavigation = [
        { name: t('navigation.dashboard'), href: '/dashboard', icon: Home, section: 'main' },
        { name: t('navigation.exclusion_lookup'), href: '/dashboard/lookup', icon: Search, section: 'main' },
        { name: t('navigation.api_keys'), href: '/dashboard/api-keys', icon: Key, section: 'main' },
        { name: t('navigation.webhooks'), href: '/dashboard/webhooks', icon: Webhook, section: 'main' },
    ]
    ...
    const insightsNavigation = [
        { name: t('navigation.statistics'), href: '/dashboard/statistics', icon: BarChart3, section: 'insights' },
        { name: t('navigation.compliance'), href: '/dashboard/compliance', icon: Shield, section: 'insights' },
    ]

    return { mainNavigation, toolsNavigation, insightsNavigation }
}
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    const { mainNavigation, toolsNavigation, insightsNavigation } = getNavigation(t)
        ...
        <div className="min-h-screen bg-gray-50 flex lg:flex-row flex-col">
            ...
            <div className={`fixed inset-y-0 left-0 z-50 w-72 bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 flex flex-col transform transition-all duration-300 lg:relative lg:translate-x-0 shadow-2xl lg:shadow-lg overflow-hidden ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'
                ...
                <nav className="flex-1 overflow-y-auto px-3 py-6 space-y-8 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-slate-600 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:hover:bg-slate-500">
                    {/* Main Navigation */}
                    <div>
                        <p className="px-4 text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 opacity-80">Core Navigation</p>
                    ...
                    {/* Tools Navigation */}
                    <div>
                        <p className="px-4 text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 opacity-80">Developer Tools</p>
                        <ul className="space-y-2">
                    ...
                    {/* Insights Navigation */}
                    <div>
                        <p className="px-4 text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 opacity-80">Analytics</p>
                        <ul className="space-y-2">
