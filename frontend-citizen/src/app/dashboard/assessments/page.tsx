...
export default function AssessmentsPage() {
    ...
    const handleStartAssessment = async (type: AssessmentType) => {
        setIsLoading(true)
        try {
            // Step 1: Start the assessment session
            const startMethod: Record<AssessmentType, () => Promise<any>> = {
                'LIEBET': () => api.post('/screening/liebet/start/', {}),
                'PGSI': () => api.post('/screening/pgsi/start/', {}),
                'DSM5': () => api.post('/screening/dsm5/start/', {})
            }

            const sessionResponse = await startMethod[type]()

            // Step 2: Get questions for the assessment type
            ...
            const questionsResponse = await api.get(`/screening/questions/type/${assessmentTypeMap[type]}/`)

            if (questionsResponse.data?.results) {
                setQuestions(questionsResponse.data.results.map((item: any) => ({
                    id: item.id,
                    text: item.text,
