...
export default function Sidebar() {
    ...
    return (
        <div className="w-64 bg-white border-r border-gray-200 flex flex-col">
            ...
            {/* Verification Status Footer for Citizens */}
            {user?.role?.includes('citizen') && (
                <div className="p-4 border-t border-gray-200 bg-gradient-to-r from-purple-50 to-blue-50">
                    <p className="text-xs font-semibold text-gray-900 mb-3">Verification Status</p>
                    <div className="space-y-2">
                        <div className="flex items-center gap-2">
                            <div className={`h-2 w-2 rounded-full ${user?.is_email_verified ? 'bg-green-500' : 'bg-yellow-500'}`} />
                            <span className="text-xs text-gray-700">Email {user?.is_email_verified ? '✓' : '⚠'}</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <div className={`h-2 w-2 rounded-full ${user?.is_phone_verified ? 'bg-green-500' : 'bg-yellow-500'}`} />
                            <span className="text-xs text-gray-700">Phone {user?.is_phone_verified ? '✓' : '⚠'}</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <div className={`h-2 w-2 rounded-full ${user?.is_id_verified ? 'bg-green-500' : 'bg-yellow-500'}`} />
                            <span className="text-xs text-gray-700">ID {user?.is_id_verified ? '✓' : '⚠'}</span>
                        </div>
                    </div>
                </div>
            )}
